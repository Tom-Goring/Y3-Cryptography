...
const D7: [u32; 6] = [4, 10, 9, 2, 1, 7];
const D8: [u32; 6] = [7, 8, 7, 1, 9, 6];
const D9: [u32; 6] = [9, 1, 7, 8, 7, 7];
const D10: [u32; 6] = [1, 2, 9, 10, 4, 1];
...
pub fn calculate_digits(weights: &[&[u32]], input: String) -> Result<String, String> {
    ...
    let input_as_digits: Result<Vec<u32>, _, > = input.chars().map(|c| c.to_digit(10).ok_or("Invalid Digit")).collect();
    ...
    let check_digits: Result<String, String> = match input_as_digits {
        Ok(digit_vec) => {
            weights
                .iter()
                .map(|weights| {
                    char::from_digit(calculate_digit(*weights, &digit_vec[..]).unwrap() as u32, 10)
